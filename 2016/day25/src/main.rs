...
fn main() -> Result<()> {
    ...
    let input_path = env::args().nth(1).ok_or_eyre("No argument provided")?;
    let program = read_input(input_path).context("Failed to read input")?;

    let clock_input = program
        .find_clock()
        .ok_or_eyre("No clock signal input found")?;
    println!("Clock signal requires A: {}", clock_input);
...
impl FromStr for Register {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        match s.as_bytes()[0] {
            ...
            b'c' => Ok(Register::C),
            b'd' => Ok(Register::D),
            _ => Err(eyre!("`{}` is not a valid register", s)),
        }
    }
}
...
#[allow(dead_code)]
impl Program {
    pub fn exec(&self) -> (State, Vec<i64>) {
        let state = State::new();
    ...
    pub fn step(
        ...
        if let Some(delta_ip) = Program::try_optimize(instructions, state, *ip) {
            *ip += delta_ip;
            return None;
        }
            ...
            Instruction::Tgl { delta } => {
                ...
                if !(idx < 0 || idx >= instructions.len() as isize) {
                    instructions[idx as usize] = match instructions[idx as usize] {
                        Instruction::Copy { src, dest } => Instruction::Jnz { src, delta: dest },
                        Instruction::Jnz { src, delta } => Instruction::Copy { src, dest: delta },
                        Instruction::Inc { r } => Instruction::Dec { r },
                        Instruction::Dec { r } => Instruction::Inc { r },
                        Instruction::Tgl { delta } => Instruction::Inc { r: delta },
                        Instruction::Out { r } => Instruction::Inc { r },
                    }
                }
            }
            Instruction::Out { r } => {
                value = Some(r.eval(state));
            }
        }
    ...
    pub fn exec_with_state(&self, mut state: State) -> (State, Vec<i64>) {
        let mut instructions = self.0.clone();
        let mut ip = 0isize;
        let mut output = Vec::new();

        while 0 <= ip && (ip as usize) < instructions.len() {
            if let Some(v) = self.step(&mut ip, &mut instructions, &mut state) {
                output.push(v);
            }
        }
    ...
    pub fn find_clock(&self) -> Option<i64> {
        for v in 0..=i64::MAX {
            let mut states = HashSet::new();
            let mut ip = 0isize;
            let mut instructions = self.0.clone();
            let mut output = Vec::new();
            let mut state = State::from_map(enum_map! {
                Register::A => v,
                Register::B => 0,
                Register::C => 0,
                Register::D => 0,
            ...
            while 0 <= ip && (ip as usize) < instructions.len() {
                if !states.insert((state, ip)) {
                    // we have entered a loop, now we check that the loop outputs the desired output
                    if !output.iter().enumerate().all(|(i, v)| (i % 2) as i64 == *v) {
                        break;
                    }
...
fn parse_input(input: impl BufRead) -> Result<Program> {
    let instructions: Vec<_> = input
        ...
        .map(|(no, line)| {
            ...
            let instr = split
                .next()
                .ok_or_else(|| eyre!("No instruction on line {}", no))?;

            match instr {
                "cpy" => {
                    let src_raw = split
                        .next()
                        .ok_or_else(|| eyre!("cpy on line {} has no arguments", no))?;
                    let dest_raw = split
                        .next()
                        .ok_or_else(|| eyre!("cpy on line {} has no destination", no))?;
                ...
                "inc" => {
                    let reg_raw = split
                        .next()
                        .ok_or_else(|| eyre!("inc on line {} has no arguments", no))?;
                ...
                "dec" => {
                    let reg_raw = split
                        .next()
                        .ok_or_else(|| eyre!("dec on line {} has no arguments", no))?;
                ...
                "jnz" => {
                    let src_raw = split
                        .next()
                        .ok_or_else(|| eyre!("jnz on line {} has no arguments", no))?;
                    let delta_raw = split
                        .next()
                        .ok_or_else(|| eyre!("jnz on line {} has no delta", no))?;
                ...
                "tgl" => {
                    let src_raw = split
                        .next()
                        .ok_or_else(|| eyre!("tgl on line {} has no arguments", no))?;
                ...
                "out" => {
                    let reg_raw = split
                        .next()
                        .ok_or_else(|| eyre!("out on line {} has no arguments", no))?;

                    let r = reg_raw
                        .parse()
                        .with_context(|| format!("out on line {} has invalid register", no))?;
...
jnz a -2";
    ...
    #[test]
    fn basic_clock() {
        let program = parse_input(Cursor::new(BASIC_CLOCK)).expect("Basic clock program parses");
        let clock_input = program.find_clock();
        assert_eq!(clock_input, Some(1));
    }
}
